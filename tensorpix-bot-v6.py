#!/usr/bin/env python3
"""
TensorPix Video Upscaler Bot v6 (FIXED + CUSTOMIZABLE)
========================================================
All-in-one: Creates accounts -> splits video -> enhances -> merges

v6 CHANGES from v5:
  - FIXED DOWNLOAD: Strategy 4 now uses aria2c (NO Authorization header to CDN)
    This was the bug: CDN returned HTTP 400 when receiving Bearer token
  - NEW: --api-keys "key1,key2,key3"  multiple Boomlify API keys as fallbacks
  - NEW: --segments N                override auto segment count
  - NEW: --resolution 2160p         select target resolution (bypasses default)
  - NEW: --model animation          select enhancement model (bypasses "4x Upscale Ultra 4")
  - All download strategies use aria2c when available, urllib as fallback

Usage:
  python3 tensorpix-bot-v6.py --input input.mp4
  python3 tensorpix-bot-v6.py --input input.mp4 --api-keys "key1,key2,key3"
  python3 tensorpix-bot-v6.py --input input.mp4 --segments 4
  python3 tensorpix-bot-v6.py --input input.mp4 --resolution 2160p --model animation
  python3 tensorpix-bot-v6.py --input input.mp4 --seg-seconds 45 --count 5
...
# Default Boomlify key (overridable via --api-keys or BOOMLIFY_API_KEY env)
DEFAULT_BOOMLIFY_KEY = "api_48c8b7c55f4c543164e915cb78f74733a2edd80fd44ffbb6f6ac126a252370dd"

BOOMLIFY_BASE = "https://v1.boomlify.com"
EMAIL_DOMAIN = "student.nondon.store"

PASSWORD = "TpixAcc2026!"
TENSORPIX = "https://app.tensorpix.ai"

DEFAULT_REFERRAL = "https://app.tensorpix.ai/register?referral=nvAo3t5bSjWIlRdA"
...
def create_inbox():
    ...
    while tried < max_keys:
        ...
        url = f"{BOOMLIFY_BASE}/api/v1/emails/create?{params}"
        ...
        try:
            ...
            if inbox_id and email_addr:
                log.info("[Boomlify] Inbox created with key ending ...%s", key[-8:])
                return inbox_id, email_addr
        except urllib.error.HTTPError as e:
            if e.code in (401, 402, 403, 429):
                log.warning("[Boomlify] Key ...%s failed (%d), trying next ...", key[-8:], e.code)
                tried += 1
                continue
            log.error("[Boomlify] HTTP error: %s", e)
        except Exception as e:
            log.error("[Boomlify] Inbox error: %s", e)
            tried += 1
            continue
    log.error("[Boomlify] All API keys exhausted or failed")
    return None, None
...
# ═══════════════════════════════════════════════════════════════════════════
# ENHANCEMENT SETTINGS (v6: customizable resolution + model)
# ═══════════════════════════════════════════════════════════════════════════

async def tpx_enhance(page):
    """Select enhancement settings and click Enhance.

    v6: Supports --resolution and --model CLI overrides.
...
# ═══════════════════════════════════════════════════════════════════════════
# DOWNLOAD  --  MULTI-STRATEGY (v6 FIX: aria2c, NO auth header to CDN)
# ═══════════════════════════════════════════════════════════════════════════
...
async def _download_strategy_api(page, save_dir, seg_idx, email, password):
    ...
    # ★★★ v6 FIX: Download CDN file WITHOUT Authorization header ★★★
...
async def tpx_download(page, save_dir, seg_idx, email, password):
    ...
    v6: All strategies use download_file() which tries aria2c first, urllib fallback.
...
async def process_segment(browser, account, seg_path, seg_idx, total,
                           save_dir, seg_duration):
    """Full pipeline: login -> upload -> enhance -> wait -> download."""
    log.info("=" * 60)
    log.info("SEGMENT %d/%d -- account: %s", seg_idx + 1, total,
             account["email"])
    log.info("=" * 60)
    ...
    try:
        ...
        log.info("[Bot] Enhancement done! Downloading ...")

        await page.wait_for_timeout(3000)
        result = await tpx_download(page, save_dir, seg_idx,
                                     account["email"], account["password"])

        if result:
            log.info("[Bot] SEGMENT %d DONE -> %s (%.1f MB)",
                     seg_idx, result, os.path.getsize(result) / (1024 * 1024))
        else:
...
async def run(args):
    log.info("=" * 60)
    log.info("TensorPix Bot v6 -- Video Enhancement Pipeline")
    log.info("=" * 60)
    ...
    if args.api_keys:
        ...
        log.info("Using %d Boomlify API key(s)", len(BOOMLIFY_API_KEYS))
    else:
        ...
        log.info("Using %d API key (default)", len(BOOMLIFY_API_KEYS))
    ...
    creds_file = os.path.join(work_dir, "tensorpix_accounts.txt")
    seg_dir = os.path.join(work_dir, "segments")
    save_dir = os.path.join(work_dir, "enhanced")
    output = os.path.abspath(args.output or os.path.join(work_dir, "enhanced_output.mp4"))
    ...
    async with async_playwright() as pw:
        ...
        for idx, seg_path in enumerate(segments):
            ...
            result = await process_segment(browser, acct, seg_path, idx,
                                           total, save_dir, seg_dur)
    ...
    # Report
    log.info("=" * 60)
    log.info("RESULTS: %d/%d segments enhanced", len(enhanced), total)
    if failed:
        log.warning("Failed segments: %s", failed)
    if enhanced:
        log.info("Merging %d segments -> %s", len(enhanced), output)
        merge_segments(enhanced, output)
    log.info("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="TensorPix Video Upscaler Bot v6",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic (default 4x Upscale Ultra 4):
  python3 tensorpix-bot-v6.py --input input.mp4
  ...
  python3 tensorpix-bot-v6.py --input input.mp4 --count 5 --segments 6 --resolution 2160p --model animation --seg-seconds 30

  # Custom output filename:
  python3 tensorpix-bot-v6.py --input input.mp4 --output my_4k_anime.mp4
        """)

    parser.add_argument("--input", required=True,
                        help="Input video file (e.g., input.mp4)")
    parser.add_argument("--output", default=None,
                        help="Output merged video (default: enhanced_output.mp4)")
    ...
    # Add log file
    log_dir = os.path.dirname(os.path.abspath(args.input))
    _add_log_file(os.path.join(log_dir, "tensorpix_v6.log"))
